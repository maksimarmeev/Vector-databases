...
for i, (question, relevant_passages_raw) in enumerate(zip(questions, relevant_passages_list), start=1):
    ...
    found_articles = [str(obj.properties['article_id']) for obj in response.objects]
    relevant_positions = {}
    ...
    print(f"Search for question {question}")    
    print("------")
...
excel_path = "D:/Embedding/Weaviate/final.xlsx"
...
plt.title('Weaviate CPU Usage During Search')
plt.legend()
...
plt.title('Weaviate Memory Usage During Search')
plt.legend()

plt.tight_layout()
plot_path = "D:/Embedding/Weaviate/weaviate_usage.png"
...
plot_path = "D:/Embedding/Weaviate/final.png"
plt.savefig(plot_path, dpi=300, bbox_inches='tight')

...
for i, (question, relevant_passages_raw) in enumerate(zip(questions, relevant_passages_list), start=1):
    ...
    found_articles = [str(hit["_id"]) for hit in response["hits"]["hits"]]
    relevant_positions = {}
    ...
    print(f"Nubmer {i} Search for question {question}")    
    print("------")
...
df = pd.DataFrame(results, columns=["id", "question", "relevant_passage_positions", "Embedding time", "Search time", "Max position"])
excel_path = "D:/Embedding/Elasticsearch/final.xlsx"
...
plt.title('Elasticsearch CPU Usage During Search')
plt.legend()
...
plt.title('Elasticsearch Memory Usage During Search')
plt.legend()

plt.tight_layout()
plot_path = "D:/Embedding/Elasticsearch/elasticsearch_usage.png"
...
plot_path = "D:/Embedding/Elasticsearch/final.png"
plt.savefig(plot_path, dpi=300, bbox_inches='tight')
